...
const ImageUploader = ({ images, onUpload, onRemove, maxImages = 4 }) => {
  ...
  const handleImageUpload = (files) => {
    ...
    // Process up to max number of images
    ...
    // Once all images are processed, update the form data
  ...
  return (
    <div className="mt-6">
      <label className="block text-sm font-medium text-gray-700 mb-2">Images (Max {maxImages})</label>
      <div
        onDragOver={(e) => {
          e.preventDefault();
          setDraggedOver(true);
        ...
        onDrop={handleDrop}
        className={`mt-1 border-2 border-dashed rounded-lg p-4 text-center ${draggedOver ? 'border-orange-500 bg-orange-50' : 'border-gray-300'}`}
      >
