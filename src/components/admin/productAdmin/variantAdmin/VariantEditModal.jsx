...
const VariantEditModal = ({
  ...
  const colorAttributes = form.attributes.filter(attr => attr.type === 'color')
  const sizeAttributes = form.attributes.filter(attr => attr.type === 'size')
  ...
  const handleDeleteAttribute = async (attributeId) => {
    try {
      ...
      setForm((prev) => ({ ...prev, attributes: prev.attributes.filter((attr) => attr.id !== attributeId) }))
    } catch (err) {
    ...
    //     setForm((prev) => (
    //         {...prev, imageFile:e.target.file[0] }
    //     ))
    // }
    ...
    if (!type || !value) return;
      try {
        ...
        if (resAttribute.data.status === "SUCCESS") {
          ...
          setForm((prev) => ({
            ...prev,
            attributes: [...prev.attributes, { type: type, value: value }],
        }))
        }
      } catch (err) {
          toast.error(`Lỗi: ${err.message} - Nguyên nhân: ${err.response.statusText} `)
  ...
  return (
    <div className="fixed inset-0 bg-[#0000009e] bg-opacity-50 flex items-center justify-center p-4 overflow-auto z-10" onClick={onClose} >
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-3">
            <label className="block text-sm font-medium mb-1">Ảnh thuộc tính</label>
          ...
          <div className="mb-3">
            ...
            <div className="flex-1">
              ...
              {colorAttributes.map((attr, idx) => (
                <div key={attr.id || idx} className="flex items-center gap-2 mb-1" >
                  <span
